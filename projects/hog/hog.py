...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    n_1 = player_score % 10
    n_2 = opponent_score // 10 % 10
    diff = abs(n_1 - n_2)
    if diff != 0:
        boar_brawl_score = diff * 3
    else:
        boar_brawl_score = 1
    return boar_brawl_score
    # END PROBLEM 2
...
def always_roll(n):
    ...
    3
    """
    assert n >= 0 and n <= 10, "Please input number of rolls from 0 to 10"
    # BEGIN PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score0 < goal:
        score1 = 0
        while score1 < goal:
            n_1 = strategy(score0, score1)
            score1 += 1
            n_2 = strategy(score0, score1)
            if not n_1 == n_2:
                 trigger = False        
        score0 += 1
    return trigger
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    scoring_sequence = []
    while n <= 10:
        scoring_num_rolls = averaged_roll_dice(n,dice)
        scoring_sequence.append(scoring_num_rolls)
        n += 1
    max_scoring_num_rolls = scoring_sequence.index(max(scoring_sequence)) + 1
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    bb_trial = boar_brawl(score, opponent_score)
    if bb_trial >= threshold:
        return 0
    else:        
